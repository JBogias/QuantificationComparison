...
plot_cor_cowplot <- function(joined_df, colour_txps = NA) {
  ...
  plot_sample_cor <- function(df, x, y) {
    ...
    cor_to_plot <- df %>%
      ...
      dplyr::mutate(colour_txps = case_when(
        transcript_id %in% head(bowtie2_loadings$transcript_id, 100) ~ "Top 100 Negative\nPC1 Loadings",
        ...
        transcript_id %in% head(sa_loadings$transcript_id, 100) ~ "Top 100 Negative\nPC11 Loadings",
  ...
  xlab <- textGrob("Transcript Expression (log2 CPM)",
  ...
  ylab <- textGrob("Transcript Expression (log2 CPM)",
                   gp = gpar(fontface = "bold",
                             col = "black",
                             fontsize = 26),
                   rot = 90)
